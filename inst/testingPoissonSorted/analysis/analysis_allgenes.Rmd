...
subtitle: "Including all genes in deconvolution"
...
full_join(d, e, by = c("detected" = "expected")) %>% 
  rename(connection = detected, n.expected = n.y, n.detected = n.x) %>%
  replace_na(list(n.expected = 0, n.detected = 0)) %>%
  select(connection, n.expected, n.detected)
