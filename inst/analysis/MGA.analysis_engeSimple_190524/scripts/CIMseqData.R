...
# )
mca <- FindClusters(
  object = mca, reduction.type = "pca", dims.use = pcs, resolution = 0.2,
  n.start = 100, n.iter = 1000, nn.eps = 0, k.param = 20, prune.SNN = 1/15,
  algorithm = 1, save.SNN = TRUE, print.output = FALSE, plot.SNN = FALSE,
  force.recalc = TRUE, random.seed = 93820
)
...
select <- match(pull(u.markers, gene), rownames(singlets))
names(select) <- pull(u.markers, cluster)
