...
mca <- CreateSeuratObject(raw.data = singlets[, loc == "distal"])
mca <- NormalizeData(
...
mca <- RunUMAP(
  ...
  n_neighbors = 15, seed.use = 7937393
)
...
cObjMul <- CIMseqMultiplets(multiplets[, loc.m == "distal"], multipletERCC[, loc.m == "distal"], select)
