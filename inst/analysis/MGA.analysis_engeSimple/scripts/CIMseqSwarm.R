...
if(!as.numeric(last3) >= 301) {
  stop("sp.scRNAseq package version too low. Must be >= 0.3.0.1")
}
...
if(!is.na(args[1])) {
  ...
  sObj <- CIMseqSwarm(
    cObjSng, cObjMul, maxiter = 100, swarmsize = swarmsize, nSyntheticMultiplets = 400,
    swarmInit = init, psoControl = list(eps.stagnate = 1, maxit.stagnate = 5)
  )
