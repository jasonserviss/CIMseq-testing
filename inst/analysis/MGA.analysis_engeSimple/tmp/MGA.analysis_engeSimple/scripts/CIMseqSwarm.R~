...
if(!is.na(args[1])) {
  ...
  cObjMul <- CIMseqMultiplets(
    matrix(counts[, i], ncol = 1, dimnames = list(rownames(counts), colnames(counts)[i])),
    matrix(counts.ercc[, i], ncol = 1, dimnames = list(rownames(counts.ercc), colnames(counts.ercc)[i])),
