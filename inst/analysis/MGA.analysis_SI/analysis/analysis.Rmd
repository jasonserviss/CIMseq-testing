...
plotUnsupervisedMarkers(
  cObjSng, cObjMul,
  c("Mki67", "Lgr5", "Muc2", "Ptprc", "Chga", "Alpi", "Lyz1", "Dclk1"), 
...
tibble(nConnections = cut1, cost = getData(sObj, "costs")) %>%
  mutate(nConnections = parse_factor(as.character(nConnections), levels = sort(unique(nConnections)))) %>%
  ggplot() + 
...
nConnections <- cut1[match(ercc$sample, names(cut1))]
tibble(
  cellNumber = ercc$estimatedCellNumber,
  nConnections = nConnections
) %>%
  ggplot() +
  geom_boxplot(aes(nConnections, cellNumber, group = nConnections)) +
  labs(x = "Connections (n)", y = "Estimated cell number") +
  scale_x_continuous(breaks = 1:max(nConnections)) +
  scale_y_continuous(breaks = 1:max(ercc$estimatedCellNumber)) +
  theme_bw()
```
