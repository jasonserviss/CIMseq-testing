...
#merge TM
iGenes <- intersect(rownames(TMD.Counts), rownames(MGA.Counts))
singlets <- cbind(MGA.Counts[iGenes, boolSng], TMD.Counts[iGenes, ])
singletERCC <- cbind(MGA.CountsERCC[, boolSng], TMD.CountsERCC)
...
mca@meta.data <- data.frame(mca@meta.data, origin = if_else(str_detect(rownames(mca@meta.data), "SRR"), "TM", "Enge"))
mca <- NormalizeData(
...
mca@dr$umap@cell.embeddings %>%
  ...
  mutate(origin = if_else(str_detect(sample, "SRR"), "TM", "Enge")) %>%
  ggplot() +
  geom_point(aes(UMAP1, UMAP2, colour = origin), alpha = 0.5)
