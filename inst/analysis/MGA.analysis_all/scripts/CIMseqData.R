...
mca@meta.data$source.full <- case_when(
  str_detect(rownames(mca@meta.data), "NJ") ~ "Enge",
  str_detect(rownames(mca@meta.data), "SRR6") ~ "Tabula muris", 
  str_detect(rownames(mca@meta.data), "SRR5") ~ "Regev",
  TRUE ~ "error"
)
...
mca <- RunHarmony(
  ...
  nclust = 30, max.iter.cluster = 100, max.iter.harmony = 100, dims.use = pcs,
  epsilon.cluster = -Inf
)
