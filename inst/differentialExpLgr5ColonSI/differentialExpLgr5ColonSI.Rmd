...
DE genes should be reported with mean expression and
...
#spUnsupervised
uObj <- spUnsupervised(cObjSng, max_iter = 7000, initial_dims = sum(s), max = 500, seed = 73223)
#save(uObj, file = "~/Github/sp.scRNAseqTesting/inst/testingPoissonAlgo/uObj.rda")
#load("~/Github/sp.scRNAseqTesting/inst/testingPoissonAlgo/uObj.rda")
```
...
mutate(newClass = case_when(
  oldClass == "C1" ~ "C.Stem",
  oldClass == "G1" ~ "C.Colonocyte",
  oldClass == "B1" ~ "C.Goblet",
  oldClass == "H1" ~ "Blood.Tufft.Endocrine",
  oldClass == "D1" ~ "SI.Stem",
  oldClass %in% c("A1", "E1") ~ "SI.Enterocyte",
  oldClass == "F1" ~ "SI.Goblet",
  TRUE ~ "error"
))

classification(uObj) <- renameClass$newClass[match(rownames(getData(uObj, "tsne")), renameClass$sample)]
...
dotplot <- function(data) {
  ...
  rename(Gene = gene) %>%
  ggplot() +
  #geom_boxplot(aes(Classification, expression, colour = Gene, group = Classification), outlier.shape = NA) +
  geom_dotplot(
  ...
  geom_text(
    ...
    aes(Classification, 0, label = do), size = 2
  ) +
  theme_few() +
  scale_colour_ptol() +
  scale_fill_ptol() +
