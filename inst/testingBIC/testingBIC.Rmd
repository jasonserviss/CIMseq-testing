...
The detected connections look much more realistic. Many of the connections are expected due to the fact that they represent known ongoing differentiation processes (Endocrine progenitor -> Beta, Ductal -> Acinar, Ductal -> Endocrine Progenitor). In addition 37 of the edges (or `r paste(round(37/sum(allEdges$weight), digits = 2), "%", sep = "")` of the total detected edges) are endothelial - endothelial connections which makes sense due to the fact that multiplets were sorted on this marker. The number of self connections in the multiplets is `r sum(filter(allEdges, from == to)$weight)` or `r paste(round(sum(filter(allEdges, from == to)$weight) / sum(allEdges$weight), digits = 2), "%", sep = "")` of the total edges. A table below shows the non-self connections, their weights and p-values (although the p-value calculation needs some improvment).
...
funx <- function(swarm, edge.cutoff = 0) {
    ...
    conns <- lapply(1:length(input), function(x) {
        y <- input[[x]]
        if(length(y) > 1) {
            co <- combn(y, 2)
        ...
        as_tibble() %>%
        rename(from = X1, to = X2, multiplet = rowname) %>%
        select(multiplet, from, to) %>%
        mutate(
            from = as.character(from), 
            to = as.character(to), 
            multiplet = as.character(multiplet)
        )
}

funx(getData(syntheticDataSwarm, "spSwarm")) %>%
