...
.determineEdgeCutoff_generateFractions_doublet <- function(
  uObj, reps = 10, interval = c(0.5, 10^-(1:5))
){
  classes <- getData(uObj, "classification")
  uClass <- sort(unique(classes))
  ...
  tibble(
    ...
    cidx = rep(rep(1:lk, each = reps), (li * lk * lc) / (length(uClass) - 1))
  ) %>%
...
#SCRIPT
set.seed(32780923)
frac <- .determineEdgeCutoff_generateFractions_doublet(uObj, reps = 1, interval = c(0.5, 10^-(1:4)))
