...
```{r syntheticCountsLoad, fig.align='center', fig.height=8, fig.width=10, eval=TRUE, message=FALSE}
s <- grepl("^s", colnames(syntheticDataTest))
cObjSng <- syntheticDataTest[,s]
cObjMul <- syntheticDataTest[,!s]

p <- spPlot(syntheticDataUnsupervised, cObjSng, type="clusters")
p 
ggsave(p, file="~/Github/sp.scRNAseqTesting/inst/connectionsTest/syntheticTestUnsupervised.pdf", device=cairo_pdf)
...
p
ggsave(p, file="~/Github/sp.scRNAseqTesting/inst/connectionsTest/syntheticTestSwarm.pdf", device=cairo_pdf)
```
...
detected <- spSwarmPoisson(syntheticDataSwarm, edge.cutoff = edge.cutoff, min.pval=min.pval, min.num.edges=min.num.edges)
real <- as.data.frame(syntheticDataTable)

d <- data.frame(
    real=sort(rep(paste(real$from, real$to, sep="-"), real$Freq)),
    detected=sort(rep(paste(detected$from, detected$to, sep="-"), detected$weight))
)

table <- as.data.frame(table(d))

p <- ggplot(table, aes(x=real, y=detected, fill=Freq))+
...
p
ggsave(p, file="~/Github/sp.scRNAseqTesting/inst/connectionsTest/syntheticTestResults.pdf", device=cairo_pdf)
```
...
```{r testCountsLoad, fig.align='center', fig.height=8, fig.width=10, eval=TRUE}
#load test data
s <- grepl("^s", colnames(expressionTestData))
cObjSng <- expressionTestData[,s]
cObjMul <- expressionTestData[,!s]

p <- spPlot(expressionTestUnsupervised, cObjSng, type="clusters")
p
ggsave(p, file="~/Github/sp.scRNAseqTesting/inst/connectionsTest/expressionTestUnsupervised.pdf", device=cairo_pdf)
...
p
ggsave(p, file="~/Github/sp.scRNAseqTesting/inst/connectionsTest/expressionTestSwarm.pdf", device=cairo_pdf)
...
detected <- spSwarmPoisson(expressionTestSwarm, edge.cutoff = edge.cutoff, min.pval=min.pval, min.num.edges=min.num.edges)
real <- as.data.frame(expressionTestTable)
...
p
ggsave(p, file="~/Github/sp.scRNAseqTesting/inst/connectionsTest/expressionTestResults.pdf")
```
...
```{r experimentalDataLoad, eval=FALSE}
#load test data
data(expData)
...
```
<br></br>
```{r, fig.align='center', fig.height=8, fig.width=10, eval=FALSE}
...
```
<br></br>
```{r experimentalUnsupervisedLoad, echo=FALSE, eval=FALSE}
uObj <- experimentalDataUnsupervised
```
<br></br>
```{r experimentalClustersPlot, fig.align='center', fig.height=8, fig.width=10, eval=FALSE}
...
```{r experimentalMarkersPlot, fig.align='center', fig.height=15, fig.width=10, eval=FALSE}
...
```
<br></br>
```{r experimentResultsLoad, echo=FALSE, eval=FALSE}
result3 <- experimentalDataSwarm
```
<br></br>
```{r experimentResultsPlot, fig.align='center', fig.height=8, fig.width=10, eval=FALSE, message=FALSE}
#plot results
spPlot(result3)
```
...
```{r sourceLoad, echo=FALSE, eval=FALSE}
