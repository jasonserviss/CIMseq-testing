...
#look at e.g. Blood - SI.TA.Mki67.medium connections, the lowest and highest
#frequency cell types
expected[expected$from %in% c("Blood", "SI.TA.Mki67.medium") & expected$to %in% c("Blood", "SI.TA.Mki67.medium"), c("from", "to", "expected.perm")]
...
class.freq <- c(table(classes))
allProbs <- expand.grid(from = names(class.freq), to = names(class.freq), stringsAsFactors = FALSE) %>%
  filter(from != to) %>%
...
```{r, fig.align="center", fig.width=10, fig.height=8}
nsig <- sum(pull(edgeStats, p.pois) < 0.05)
cols <- viridis::viridis(nsig + 1)
pdata <- edgeStats %>%
  mutate(significant = if_else(p.pois < 0.05, TRUE, FALSE)) %>%
  mutate(idx = ntile(-p.pois, nrow(.))) %>%
  mutate(idx = idx - max(idx[p.pois > 0.05])) %>%
...
pdata %>%
  ...
  scale_fill_identity(
    ...
    labels = c(format(sort(filter(pdata, significant)$p.pois), digits = 2, scientific = TRUE), "n.s.")) +
  theme_bw() +
  theme(
