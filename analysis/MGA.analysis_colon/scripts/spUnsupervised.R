...
max.dim <- 10
mca <- FindClusters(
  object = mca, reduction.type = "pca", dims.use = 1:max.dim, resolution = 3,
  save.SNN = TRUE, n.start = 100, nn.eps = 0.5, print.output = FALSE,
  force.recalc = TRUE
)
mca <- RunUMAP(
  object = mca, reduction.use = "pca", dims.use = 1:max.dim, min_dist = 0.4,
  n_neighbors = 15
)
mca <- RunTSNE(object = mca, dims.use = 1:max.dim, do.fast = TRUE)
...
FeaturePlot(
  mca,
  c("Lgr5", "Ptprc", "Chga", "Dclk1", "Slc26a3", "Atoh1", "Hoxb13", "Mki67"),
