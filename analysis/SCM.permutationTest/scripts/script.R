...
if(file.exists(file.path(currPath, '../SCM.analysis/data/CIMseqData.rda'))) {
  load(file.path(currPath, '../SCM.analysis/data/CIMseqData.rda'))
}
...
perms <- map(1:nPerms, function(x) {
  ...
  CIMseqSwarm(
    cObjSng, cObjMul, maxiter = 100, swarmsize = 500,
