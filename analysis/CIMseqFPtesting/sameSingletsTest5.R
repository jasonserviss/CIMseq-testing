...
class <- rep(LETTERS[1:3], each = length(s))
class <- c(class, rep("HOS", length = length(s2)))

#make CIMseqSinglets object
cObjSng2 <- CIMseqSinglets(
  getData(cObjSng, "counts")[, c(s, s, s, s2)],
  getData(cObjSng, "counts.ercc")[, c(s, s, s, s2)],
  getData(cObjSng, "dim.red")[c(s, s, s, s2), ],
  class
)
...
#save
save(cObjSng2, cObjMul2, sObj2, file = './output5.rda')
