...
#' @param multiplets data.frame. The multiplets data.frame or a empty data.frame
...
#' adjustMultuplets
#'
#' This unit adjusts the multiplets so that each cell type has one prevelant
...
adjustMultuplets <- function(
    ...
    current <- quantifyConnections(colnames(multiplets))
...
synthesizeAndAdd <- function(
    ...
    mat <- matrix(unlist(quadruplets[, quadrupletIdx]), nrow = nrow(multiplets))
...
#' @param i integer. Current iteration.
#' @param currentTypes data.frame. Output from the quantifyConnections unit.
#' @param multiplets data.frame. The multiplets data.frame.
#' @param targetConnections data.frame. Output from the decideConnections unit.
...
checkFrequency <- function(
    ...
    after <- quantifyConnections(colnames(multiplets))
...
adjustSelf <- function(
    ...
    conn <- quantifyConnections(colnames(multiplets))
    ...
    self <- data.frame(row.names = 1:nrow(multiplets))
...
plotMultipletsCellNrs <- function(
    multipletNames
){
